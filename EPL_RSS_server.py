...
import sys, os, re, imp, stat, shutil
...
TRACKERS=['udp://tracker.openbittorrent.com:80','udp://tracker.publicbt.com:80']
...
class Servidor(QtWidgets.QMainWindow):
    ...
    def on_BotonExaminar_clicked(self):
        ...
        self.fl = QtWidgets.QFileDialog.getOpenFileName(self, 'Seleccionar archivo RSS o CSV',ruta, 'Archivos RSS o CSV (*.rss *.csv)')[0]
        if self.fl:
    ...
    def on_BotonRutaDirectorio_clicked(self):
        ...
        dir = QtWidgets.QFileDialog.getExistingDirectory(self,'Seleccionar directorio',ruta,QtWidgets.QFileDialog.ShowDirsOnly|QtWidgets.QFileDialog.DontResolveSymlinks|QtWidgets.QFileDialog.HideNameFilterDetails|QtWidgets.QFileDialog.ReadOnly)
        if dir:
            self.dir_biblio= os.path.normpath(dir)
            self.BotonRutaDirectorio.setText(os.path.normpath(dir))
            self.BotonRutaBiblioteca.setText(os.path.normpath(dir))
    ...
    def on_BotonDirDestino_clicked(self):
        ...
        dir = QtWidgets.QFileDialog.getExistingDirectory(self,'Seleccionar directorio',ruta,QtWidgets.QFileDialog.ShowDirsOnly|QtWidgets.QFileDialog.DontResolveSymlinks|QtWidgets.QFileDialog.HideNameFilterDetails|QtWidgets.QFileDialog.ReadOnly)
        if dir:
            self.EditDirDestino.setText(os.path.normpath(dir))
    ...
    def on_radioMover_clicked(self):
        if self.radioMover.isChecked():
            ...
            dir = QtWidgets.QFileDialog.getExistingDirectory(self,'Seleccionar directorio para los duplicados',ruta,QtWidgets.QFileDialog.ShowDirsOnly|QtWidgets.QFileDialog.DontResolveSymlinks|QtWidgets.QFileDialog.HideNameFilterDetails|QtWidgets.QFileDialog.ReadOnly)
            if dir:
                if (os.path.normpath(dir)!=self.dir_biblio):
                    self.dir_duplicados = os.path.normpath(dir)
                    self.radioMover.setToolTip('Mover la versión más antigua del fichero al directorio '+self.dir_duplicados)
                else:
            ...
            try:
                ...
                if extension.lower()=='.rss':
                    ...
                    if extension.lower()=='.csv':
                        ...
                        self.muestra_mensaje('El tipo de archivo cargado es incorrecto (sólo se admiten RSS y CSV)','red')
                        exito = False
                if exito:
                ...
                if libro:
                    try:
                        if "enlace" in libro:
                            ...
                            enlace+= str(normalize('NFKD', str(libro["titulo"])).encode('ASCII', 'ignore'), encoding='UTF-8')
                            enlace+= self.trackers_usar
                            if sys.platform=="win32":
                                if self.cliente!='':
                                    if (self.EditDirDestino.text() != '') and ('utorrent' in self.cliente.lower()):
                                        subprocess.Popen([self.cliente,'/DIRECTORY', os.path.normpath(self.EditDirDestino.text()), enlace])
                                    else:
                                ...
                                try:
                                    if (self.cliente!='') and (os.path.isfile(self.cliente)):
                                        if ((self.EditDirDestino.text() != '') and (os.path.isdir(self.EditDirDestino.text()))
                                            and ('utorrent' in os.path.normcase(self.cliente))):
                                            subprocess.Popen([self.cliente,'/DIRECTORY', self.EditDirDestino.text(), enlace])
                                        else:
                                            subprocess.Popen([self.cliente, enlace])
                                    ...
                                    self.muestra_mensaje('No fue posible usar el programa asociado a los archivos torrent', 'red')
                                    self.pausar_proceso()
                                    return
                        if "titulo" in libro:
    ...
    def pausar_proceso(self):
        ...
        if self.procesando==True:
            self.timer.stop()
            self.procesando = False
            self.muestra_mensaje('Envío de enlaces al programa torrent en pausa (pulsar el botón "Continuar" para reanudar)','darkgreen')
            # Habilitar / Deshabilitar botones
    ...
    def reiniciar_proceso(self, fin_normal=False):
        ...
        if fin_normal:
            ...
            self.AreaMensajes.clear()
            self.muestra_mensaje('Envío de enlaces al programa torrent reiniciado (pulsar el botón "Comenzar" para empezar de nuevo)', 'darkgreen')
    ...
    def guardar_estado(self, archivo="estado.sta"):
        ...
        try:
            ...
            self.muestra_mensaje('Se ha producido un error al intentar guardar el estado del programa en el disco:', 'red')
    ...
    def cargar_estado(self, archivo="estado.sta"):
        ...
        try:
            try:
                ...
                self.muestra_mensaje('Se ha producido un error al intentar cargar el estado anterior del programa desde el disco:', 'red')
                self.muestra_mensaje(str(ex),'red')
                return
        ...
        try:
            ...
            if canal is not None:
                ...
                if 'Se ha superado el número de descargas diarias' in descripcion:
                    self.muestra_mensaje('El archivo descargado no es válido por haber superado el límite de descargas diario:', 'red')
            ...
            midom.unlink()
            self.muestra_mensaje('Archivo cargado con éxito ('+str(len(self.lista_libros))+' enlaces encontrados)', 'darkgreen')
            exito = True
        except Exception as ex:
                        ...
                        try:
                            ...
                            for aut in autores:
                                ...
                                for i in range(len(p_autor)-1,-1,-1):
                                    if nom_autor!='': nom_autor+=' '
                                    nom_autor+= p_autor[i].strip()
                                if nom_autores!='': nom_autores+=' & '
    ...
    def borrar_antiguos(self):
        ...
        try:
            while (actual < len(self.lista_archivos)):
                ...
                if (self.CoincSinId.isChecked()) or ('ID' in self.lista_archivos[actual]):
                    for i in range(actual+1,len(self.lista_archivos)-1):
                        if (('ext' in self.lista_archivos[actual]) and ('ext' in self.lista_archivos[i]) and
                            (self.lista_archivos[actual]['ext'] == self.lista_archivos[i]['ext'])):
                            if (('ID' in self.lista_archivos[i]) and ('ID' in self.lista_archivos[actual]) and
                                (self.lista_archivos[actual]['ID'] == self.lista_archivos[i]['ID'])):
                                if (self.RevisarTitulos.isChecked()):
                                    if (self.comparar_titulo_autor(self.lista_archivos[actual]['titulo_comp'],'',self.lista_archivos[i]['titulo_comp'],'')):
                                        ...
                                        elegido = QtWidgets.QMessageBox.question(None, "Coincidencia parcial",
                                            ...
                                            +self.lista_archivos[actual]['titulo_comp']+"\r\n"+self.lista_archivos[i]['titulo_comp']+"\r\n"
                                     ...
                                     coincidencias.append(i)
                            elif self.CoincSinId.isChecked() and ((not 'ID' in self.lista_archivos[i]) or (not 'ID' in self.lista_archivos[actual])):
            ...
            self.muestra_mensaje('Se ha producido el siguiente error durante la búsqueda de las versiones antiguas:', 'red')
            self.muestra_mensaje(str(ex), 'red')
        if self.radioEliminar.isChecked():
            self.muestra_mensaje('Búsqueda de versiones antiguas finalizada ('+str(self.num_borrados)+' archivos eliminados)', 'darkgreen')
        else:
            self.muestra_mensaje('Búsqueda de versiones antiguas finalizada ('+str(self.num_borrados)+' archivos movidos)', 'darkgreen')
        ...
        for i in range(len(elem_borrar)-1,-1,-1):
            ...
            try:
                ...
                if self.radioEliminar.isChecked():
                    ...
                    self.muestra_mensaje('Se ha eliminado correctamente el archivo '+self.lista_archivos[elem_borrar[i]]['nombre'],'blue')
                else:
                    ...
                    self.muestra_mensaje('Se ha movido correctamente el archivo '+self.lista_archivos[elem_borrar[i]]['nombre'],'blue')
                self.num_borrados+=1
            except Exception as ex:
                self.muestra_mensaje('Se ha producido un error al intentar eliminar o mover el archivo '+self.lista_archivos[elem_borrar[i]]['nombre']+':','red')
                self.muestra_mensaje(str(ex), 'red')
                        ...
                        if self.lista_libros[i]['epl_id'] == self.lista_archivos[j]['ID']:
                            if ('version' in self.lista_libros[i] and 'version' in self.lista_archivos[j] and
                                ...
                                if (self.comparar_titulo_autor(self.lista_libros[i]['titulo'],self.lista_libros[i]['autor'],
                                    self.lista_archivos[j]['titulo'],self.lista_archivos[j]['autor'])):
                                    ...
                                    msj+= 'En RSS/CSV : '+self.lista_libros[i]['autor']+' - '+self.lista_libros[i]['titulo']+'\n'
                                    msj+= 'Descargado : '+self.lista_archivos[j]['autor']+' - '+self.lista_archivos[j]['titulo']
